...
sudo chroot "${ROOT}" /usr/bin/apt-get -y install texinfo python3-dev flex bison libexpat1-dev
sudo chroot "${ROOT}" /usr/bin/apt-get clean
sudo chroot "${ROOT}" /bin/bash -c "cd /tmp/binutils-gdb && ./configure -disable-werror --with-guile=no -with-python=/usr/bin/python3"
